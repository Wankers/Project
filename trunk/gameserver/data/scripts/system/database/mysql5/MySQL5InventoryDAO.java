...
public class MySQL5InventoryDAO extends InventoryDAO {
	...
	public static final String SELECT_QUERY = "SELECT `item_unique_id`, `item_id`, `item_count`, `item_color`, `item_creator`, `expire_time`, `activation_count`, `is_equiped`, `is_soul_bound`, `is_seal`, `slot`, `enchant`, `item_skin`, `fusioned_item`, `optional_socket`, `optional_fusion_socket`, `charge` FROM `inventory` WHERE `item_owner`=? AND `item_location`=? AND `is_equiped`=?";
	public static final String INSERT_QUERY = "INSERT INTO `inventory` (`item_unique_id`, `item_id`, `item_count`, `item_color`, `item_creator`, `expire_time`, `activation_count`, `item_owner`, `is_equiped`, `is_soul_bound`, `is_seal`, `slot`, `item_location`, `enchant`, `item_skin`, `fusioned_item`, `optional_socket`, `optional_fusion_socket`, `charge`) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	public static final String UPDATE_QUERY = "UPDATE inventory SET  item_count=?, item_color=?, item_creator=?, expire_time=?, activation_count=?,item_owner=?, is_equiped=?, is_soul_bound=?, is_seal=?, slot=?, item_location=?, enchant=?, item_skin=?, fusioned_item=?, optional_socket=?, optional_fusion_socket=?, charge=? WHERE item_unique_id=?";
	public static final String DELETE_QUERY = "DELETE FROM inventory WHERE item_unique_id=?";
	public static final String DELETE_CLEAN_QUERY = "DELETE FROM inventory WHERE item_owner=? AND (item_location=0 OR item_location=1)";
	...
	private Item constructItem(final int storage, ResultSet rset) throws SQLException {
		...
		Item item = new Item(itemUniqueId, itemId, itemCount, itemColor, itemCreator, expireTime, activationCount,
			isEquiped == 1, isSoulBound == 1, unSeal, slot, storage, enchant, itemSkin, fusionedItem, optionalSocket,
	...
	private boolean insertItems(Connection con, Collection<Item> items, Integer playerId, Integer accountId,
		...
		try {
			...
			for (Item item : items) {
				...
				stmt.setInt(11, item.getUnSeal());
				stmt.setInt(12, item.getEquipmentSlot());
	...
	private boolean updateItems(Connection con, Collection<Item> items, Integer playerId, Integer accountId,
		...
		try {
			...
			for (Item item : items) {
				...
				stmt.setInt(9, item.getUnSeal());
				stmt.setInt(10, item.getEquipmentSlot());
				...
				stmt.setInt(18, item.getObjectId());
				stmt.addBatch();
			}
