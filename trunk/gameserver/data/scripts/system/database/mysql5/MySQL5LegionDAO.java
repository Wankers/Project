...
import gameserver.model.team.legion.LegionWarehouse;
...
public class MySQL5LegionDAO extends LegionDAO {
	...
	/** Storage Queries **/
	private static final String SELECT_STORAGE_QUERY = "SELECT `item_unique_id`, `item_id`, `item_count`, `item_color`, `item_creator`, `expire_time`, `activation_count`, `is_equiped`, `is_seal`, `slot`, `enchant`, `item_skin`, `fusioned_item`, `optional_socket`, `optional_fusion_socket`, `charge` FROM `inventory` WHERE `item_owner`=? AND `item_location`=? AND `is_equiped`=?";
	/** History Queries **/
    ...
    private static final String INSERT_WAREHOUSE_HISTORY_QUERY = "INSERT INTO legion_warehouse_history(`legion_id`, `date`, `history_type`, `name`, `item`) VALUES (?, ?, ?, ?, ?)";
    private static final String SELECT_WAREHOUSE_HISTORY_QUERY = "SELECT * FROM `legion_warehouse_history` WHERE legion_id=? ORDER BY date ASC;";
    private static final String DELETE_WAREHOUSE_HISTORY_QUERY = "DELETE FROM legion_warehouse_history WHERE legion_id = ? AND date = ?";
	/** Legion Kinah **/
	private static final String SELECT_LEGION_KINAH_QUERY = "SELECT `count` FROM `legion_kinah` WHERE `legion_id` = ?";
	private static final String UPDATE_LEGION_KINAH_QUERY = "UPDATE `legion_kinah` SET `count` = ? WHERE `legion_id` = ?";
	private static final String INSERT_LEGION_KINAH_QUERY = "INSERT INTO legion_kinah(`legion_id`) VALUES (?)";
	/**
	 * {@inheritDoc}
	 */
	@Override
		...
		DB.select(SELECT_STORAGE_QUERY, new ParamReadStH() {
			...
			public void handleRead(ResultSet rset) throws SQLException {
				while (rset.next()) {
					...
					int isSeal = rset.getInt("is_seal");
					...
					Item item = new Item(itemUniqueId, itemId, itemCount, itemColor, itemCreator, expireTime, activationCount,
						isEquiped == 1, false, isSeal, slot, storage, enchant, itemSkin, fusionedItem, optionalSocket,
	...
	@Override
	public long loadLegionKinah(final Legion legion) {
		final int legionId = legion.getLegionId();
        final long kinah = 0;
		DB.select(SELECT_LEGION_KINAH_QUERY, new ParamReadStH() {
	...
	@Override
	public void setLegionKinah(final Legion legion, final long count) {
		DB.insertUpdate(UPDATE_LEGION_KINAH_QUERY, new IUStH() {

			@Override
			public void handleInsertUpdate(PreparedStatement stmt) throws SQLException {
				
				stmt.setLong(1, count);
				stmt.setInt(2, legion.getLegionId());
				stmt.execute();
			}
		});
	}
	...
	@Override
	public boolean saveNewLegionKinah(final Legion legion) {
		boolean success = DB.insertUpdate(INSERT_LEGION_KINAH_QUERY, new IUStH() {
    ...
    @Override
	public void loadLegionWarehouseHistory(final Legion legion) {

		final Collection<LegionHistory> history = legion.getLegionWarehouseHistory();

		DB.select(SELECT_WAREHOUSE_HISTORY_QUERY, new ParamReadStH() {
			...
			public void handleRead(ResultSet resultSet) throws SQLException {
				while (resultSet.next()) {
					history.add(new LegionHistory(LegionHistoryType.valueOf(resultSet.getString("history_type")),
                        resultSet.getString("name"), resultSet.getString("item"), resultSet.getTimestamp("date")));
				}
			}
		});
	}
	...
	@Override
	public boolean saveNewLegionWarehouseHistory(final int legionId, final LegionHistory legionHistory) {
		boolean success = DB.insertUpdate(INSERT_WAREHOUSE_HISTORY_QUERY, new IUStH() {
			...
			public void handleInsertUpdate(PreparedStatement preparedStatement) throws SQLException {
				...
				preparedStatement.setString(3, legionHistory.getLegionHistoryType().toString());
				preparedStatement.setString(4, legionHistory.getName());
                                preparedStatement.setString(5, legionHistory.getItem());
				preparedStatement.execute();
			}
		});
		return success;
       ...
       @Override
	public void removeOldLegionWarehouseHistory(int legionId, Timestamp unixTime) {
		PreparedStatement statement = DB.prepareStatement(DELETE_WAREHOUSE_HISTORY_QUERY);
		try {
		    statement.setInt(1, legionId);
                    statement.setTimestamp(2, unixTime);
                    statement.execute();
			...
			log.error("Some crap, can't set int parameter to PreparedStatement[OldLegionWarehouse]History", e);
		}
		DB.executeUpdateAndClose(statement);
	}
}
