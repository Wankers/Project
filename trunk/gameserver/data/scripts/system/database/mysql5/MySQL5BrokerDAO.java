		...
		DB.select("SELECT * FROM inventory WHERE `item_location` = 126", new ReadStH() {
			...
			public void handleRead(ResultSet rset) throws SQLException {
				while (rset.next()) {
					...
					brokerItems.add(new Item(itemUniqueId, itemId, itemCount, itemColor, itemCreator, expireTime,
						activationCount, false, false, unSeal, slot, location, enchant, itemSkin, fusionedItem, optionalSocket,
