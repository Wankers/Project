...
import instance.crucible.CrucibleInstance;
...
import gameserver.network.aion.serverpackets.SM_INSTANCE_STAGE_INFO;
...
/**
 * @author xTz
 * @modified Akulatraxas of Valholl-Online: mob for asmo / ely
 */
...
public class EmpyreanCrucibleInstance extends CrucibleInstance {
	...
	public void onEnterInstance(Player player) {
		...
		if (!instanceReward.containPlayer(object)) {
			...
			getPlayerReward(object).setPlayer(player);
		}
		...
		instance.doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				if (player.isOnline()) {
					CruciblePlayerReward playerReward = getPlayerReward(player.getObjectId());
					if (nameId != 0) {
						PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1400237, new DescriptionId(nameId * 2 + 1),
							points));
					}
	...
	public void onDie(Npc npc) {
		...
		EmpyreanStage es = getEmyereanStage(npc);
		int point = 0;
		switch (npc.getNpcId()) {
			case 217511:
			case 217512:
		...
		switch (npc.getNpcId()) {
			...
			case 217477 : 
			    despawnNpc(npc);
                if (stageType == StageType.START_STAGE_1_ROUND_1 && getNpc(217480) == null) {
                    startStage1Round2();
                }
			    break;
			case 217489:
			...
			case 217480:
                despawnNpc(npc);
                if (stageType == StageType.START_STAGE_1_ROUND_1 && getNpc(217477) == null) {
                    startStage1Round2();
                }
                break;
			case 217492:
			...
			case 217508:
				...
				switch (stageType) {
					...
					case START_STAGE_2_ROUND_4:
						if (instance.getNpc(217505) == null) {
							startStage4Round4_1();
						}
						break;
				}
			...
			case 217514:
				despawnNpc(npc);
				if (getNpcs(217511).isEmpty() && getNpcs(217512).isEmpty() && getNpcs(217513).isEmpty()
			...
			case 217518:
				despawnNpc(npc);
				if (getNpcs(217515).isEmpty() && getNpcs(217516).isEmpty() && getNpcs(217517).isEmpty()
			...
			case 217522:
				despawnNpc(npc);
				if (getNpcs(217519).isEmpty() && getNpcs(217520).isEmpty() && getNpcs(217521).isEmpty()
			...
			case 217526:
				despawnNpc(npc);
				if (getNpcs(217523).isEmpty() && getNpcs(217524).isEmpty() && getNpcs(217525).isEmpty()
			...
			case 217569:
			case 217570:
				// to do when die 217570 spawn 211984
				despawnNpc(npc);
				if (stageType == StageType.START_STAGE_6_ROUND_2 && isDoneStage6Round2 && getNpcs(217569).isEmpty()
			...
			case 217584:
                ...
                sp(217585, 1785.9088f, 776.44025f, 469.5057f, (byte) 32, 6000); 
                break;
			case 217580:
                despawnNpc(npc);
                sendEventPacket(StageType.START_STAGE_7_ROUND_4, 2000);
                sp(217581, 1785.9088f, 776.44025f, 469.5057f, (byte) 32, 6000); 
                break;
			case 217585:
                despawnNpc(npc);
                sendEventPacket(StageType.START_STAGE_7_ROUND_5, 2000);
                sp(217587, 1800.9485f, 785.81647f, 469.3503f, (byte) 51, 6000);
                break;
			case 217581:
                despawnNpc(npc);
                sendEventPacket(StageType.START_STAGE_7_ROUND_5, 2000);
                sp(217586, 1800.9485f, 785.81647f, 469.3503f, (byte) 51, 6000);
                break;
			case 217587:
			case 217586:
                despawnNpc(npc);
                sendEventPacket(StageType.PASS_GROUP_STAGE_7, 2000);
                sp(205341, 1783.0873f, 796.8426f, 469.35013f, (byte) 0, 6000);
                break;
			case 217589:
				despawnNpc(npc);
				sendEventPacket(StageType.START_STAGE_8_ROUND_2, 2000);
				sp(217590, 1794.9225f, 1756.2131f, 304.1f, (byte) 55, 6000);
				break;
			case 217590:
				despawnNpc(npc);
				sendEventPacket(StageType.START_STAGE_8_ROUND_3, 2000);
				sp(217591, 1791.1407f, 1777.92f, 304.1f, (byte) 76, 6000);
				break;
			case 217591:
				despawnNpc(npc);
				sendEventPacket(StageType.START_STAGE_8_ROUND_4, 2000);
				sp(217592, 1764.3282f, 1744.4377f, 304.1f, (byte) 18, 6000);
				break;
			case 217592:
				despawnNpc(npc);
				sendEventPacket(StageType.START_STAGE_8_ROUND_5, 2000);
				sp(217593, 1786.7078f, 1757.4915f, 304.1f, (byte) 49, 6000);
				break;
			case 217593:
				despawnNpc(npc);
				sendEventPacket(StageType.PASS_GROUP_STAGE_8, 2000);
				sp(205342, 1779.5253f, 1767.4752f, 304.1f, (byte) 92, 6000);
				break;
			case 217594:
				despawnNpc(npc);
				sendEventPacket(StageType.START_STAGE_9_ROUND_2, 2000);
				sp(217595, 1328.4663f, 1711.2297f, 317.6f, (byte) 44, 6000);
				break;
			case 217595:
				despawnNpc(npc);
				sendEventPacket(StageType.START_STAGE_9_ROUND_3, 2000);
				sp(217596, 1317.1527f, 1754.2035f, 316.9f, (byte) 80, 9000);
				sp(217597, 1294.7261f, 1721.1761f, 317.3f, (byte) 19, 9000);
				break;
			case 217596:
			case 217597:
				despawnNpc(npc);
				if (getNpcs(217596).isEmpty() && getNpcs(217597).isEmpty()) {
					...
					sp(217598, 1311.5238f, 1755.2079f, 317.1f, (byte) 97, 5400);
				}
				break;
			case 217598:
				despawnNpc(npc);
				sendEventPacket(StageType.START_STAGE_9_ROUND_5, 2000);
				sp(217599, 1304.2659f, 1722.2467f, 316.5f, (byte) 23, 6000);
				break;
			case 217599:
				despawnNpc(npc);
				sendEventPacket(StageType.PASS_GROUP_STAGE_9, 2000);
				sp(205343, 1304.221f, 1737.3604f, 316.1f, (byte) 10, 6000);
				break;
			case 217600:
			case 217601:
			case 217602:
				...
				if (getNpcs(217600).isEmpty() && getNpcs(217601).isEmpty() && getNpcs(217602).isEmpty()) {
					...
					sp(217603, 1744.6332f, 1280.0349f, 394.1f, (byte) 9, 6000);
					sp(217604, 1744.7721f, 1288.3304f, 394.1f, (byte) 119, 6000);
					sp(217605, 1746.3641f, 1295.0437f, 394.1f, (byte) 112, 6000);
					sp(217606, 1767.1664f, 1289.5667f, 394.1f, (byte) 67, 20000);
				}
				break;
			case 217603:
			case 217604:
			...
			case 217606:
				despawnNpc(npc);
				if (getNpcs(217603).isEmpty() && getNpcs(217604).isEmpty() && getNpcs(217605).isEmpty()
					&& getNpcs(217606).isEmpty()) {
					sendEventPacket(StageType.START_STAGE_10_ROUND_3, 2000);
					sp(217607, 1771.2675f, 1304.7964f, 394.1f, (byte) 82, 6000);
				}
				break;
			case 217607:
				despawnNpc(npc);
				sendEventPacket(StageType.START_STAGE_10_ROUND_4, 2000);
				sp(217608, 1754.8065f, 1303.702f, 394.1f, (byte) 100, 6000);
				break;
			case 217608:
				despawnNpc(npc);
				sendEventPacket(StageType.START_STAGE_10_ROUND_5, 2000);
				sp(217609, 1765.6692f, 1288.092f, 394.1f, (byte) 30, 6000);
				break;
			case 217609:
				despawnNpc(npc);
				sendEventPacket(StageType.PASS_GROUP_STAGE_10, 2000);
				sp(205344, 1764.8821f, 1284.2795f, 394.23758f, (byte) 33, 2000);
				break;
		}
	}
	...
	public boolean onDie(final Player player, Creature lastAttacker) {
		...
		int place = 0;
		if (isInZone(ZoneName.ILLUSION_STADIUM_6_300300000, player)) { // stage 1 ~ 4
			place = 1;
		}
		else if (isInZone(ZoneName.ILLUSION_STADIUM_300300000, player)) { // stage 5
			place = 2;
		}
		else if (isInZone(ZoneName.ILLUSION_STADIUM_7_300300000, player)) { // stage 6
			place = 3;
		}
		else if (isInZone(ZoneName.ILLUSION_STADIUM_4_300300000, player)) { // stage 7
			place = 4;
		}
	...
	private EmpyreanStage getEmyereanStage(Npc npc) {
	...
	@Override
	public void onChangeStage(final StageType type) {//TODO
		switch (type) {
			case START_STAGE_1_ELEVATOR:
    			...
    			if(getPlayerRace() == Race.ASMODIANS)
                {
                    round.add(217486);
                    round.add(217489);
			...
			case START_STAGE_7_ROUND_1:
                ...
                {
                	sp(217578, 1794.908f, 811.9936f, 469.3501f, (byte) 80, 6000);
                }
				break;
			case START_STAGE_8:
				sp(205429, 1253.0679f, 829.8279f, 358.60562f, (byte) 106);
				sp(205335, 1771.407f, 1762.9862f, 303.6954f, (byte) 102);
				teleport(1776.4169f, 1749.9952f, 303.69553f, (byte) 0);
				sendEventPacket(type, 1000);
				break;
			case START_STAGE_8_ROUND_1:
				sendEventPacket(type, 2000);
				sp(217589, 1758.1841f, 1770.473f, 303.7f, (byte) 107, 6000);
				break;
			case START_STAGE_9:
				sp(205430, 1335.1913f, 1761.2654f, 319.57446f, (byte) 80);
				sp(205336, 1296.0914f, 1730.2898f, 316.72617f, (byte) 2);
				teleport(1328.935f, 1742.0771f, 316.74188f, (byte) 0);
				sendEventPacket(type, 1000);
				break;
			case START_STAGE_9_ROUND_1:
				sendEventPacket(type, 2000);
				sp(217594, 1282.5399f, 1755.8711f, 317.4f, (byte) 105, 6000);
				break;
			case START_STAGE_10:
				sp(205431, 1755.709f, 1253.4136f, 394.2378f, (byte) 33);
				sp(205337, 1766.5986f, 1291.2572f, 394.23755f, (byte) 82);
				teleport(1760.9441f, 1278.033f, 394.23764f, (byte) 0);
				sendEventPacket(type, 1000);
				break;
			case START_STAGE_10_ROUND_1:
				sendEventPacket(type, 2000);
				sp(217600, 1771.213f, 1302.0781f, 394.3f, (byte) 82, 6000);
				sp(217601, 1774.4563f, 1302.1516f, 394.3f, (byte) 82, 6000);
				sp(217602, 1765.1488f, 1305.1216f, 394.3f, (byte) 84, 6000);
				break;
		}
	}
	...
	private void startStage1Round3() {
        ...
        {
            round.add(217478);
            sp(217478, 334.844f, 339.92618f, 96.09094f, (byte) 18, 6000);
        }
	...
	private void startStage1Round4() {
        ...
        {
            round.add(217482);
            sp(217482, 341.03156f, 361.04315f, 96.09093f, (byte) 90, 6000);
        }
	...
	private void startStage1Round5() {
        ...
        {
            sp(217484, 332.093f, 349.36847f, 96.090935f, (byte) 0, 8000);
        }
	...
	public void onLeaveInstance(Player player) {
		player.getInventory().decreaseByItemId(186000124, 1);
		getPlayerReward(player.getObjectId()).setPlayerLeave();
	}
