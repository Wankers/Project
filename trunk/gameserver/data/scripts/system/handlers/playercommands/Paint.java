 ...
 */
public class Paint extends ChatCommand {
	...
	public void execute(Player player, String... params) {
		...
		if (params.length == 0 || params.length > 2) {
			PacketSendUtility.sendMessage(player, "syntax .paint <paint color|hex color|no>");
			return;
		}
		...
		try {
			...
			if (!color.equalsIgnoreCase("no")) {
				PacketSendUtility.sendMessage(player, "[Paint] Can't understand: " + color);
				return;
			}
		}
		...
		if (target.getObjectId() != player.getObjectId()) {
			PacketSendUtility.sendMessage(target, "You have been paint by " + player.getName() + "!");
		}
		PacketSendUtility.sendMessage(player, "Painted " + target.getName() + " successfully!");
	}
