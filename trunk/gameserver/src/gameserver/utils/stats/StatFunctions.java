...
import gameserver.model.templates.item.WeaponStats;
import gameserver.model.templates.item.WeaponType;
import gameserver.model.templates.npc.NpcRating;
...
public class StatFunctions {
	...
	public static int calculatePhysicalAttackDamage(Creature attacker, Creature target, Func func,
		...
		if (attacker instanceof Player)
		{
			...
			if (weapon != null) 
			{
				...
				resultDamage = pAttack.getBonus() + getMovementModifier(attacker, StatEnum.PHYSICAL_ATTACK, pAttack.getCurrent());
		...
		else if(attacker instanceof Summon)
		{
			...
			int max = Math.round((baseDamage * attacker.getLevel()) / 10);
			int min = Math.round(max * 0.8f);
		...
		if (func != null) {
			switch (func) {
				case ADD:
					skillDamages *= attacker.getGameStats().getPower().getCurrent()*0.01f;
		...
		//add bonus damage
		if (attacker instanceof Player && attacker.isInState(CreatureState.POWERSHARD)) {
			Item powerShard;
			Equipment equipment = ((Player) attacker).getEquipment();
			if (isMainHand)
				powerShard = equipment.getMainHandPowerShard();
	...
	public static boolean calculatePhysicalParryRate(Creature attacker, Creature attacked) {
		...
		if (attacked instanceof Player)
			parry = getMovementModifier((Player) attacked, StatEnum.PARRY, parry);
