...
import gameserver.model.team.legion.LegionHistoryType;
import gameserver.model.team.legion.LegionMember;
import gameserver.model.team.legion.LegionMemberEx;
import gameserver.model.team.legion.LegionPermissionsMask;
import gameserver.model.team.legion.LegionRank;
...
import gameserver.network.aion.serverpackets.SM_LEGION_SEND_EMBLEM_DATA;
...
public class LegionService {
	...
	private void storeLegion(Legion legion, boolean newLegion) {
		if (newLegion) {
			...
			DAOManager.getDAO(LegionDAO.class).saveNewLegionKinah(legion);
		}
		else {
			DAOManager.getDAO(LegionDAO.class).storeLegion(legion);
			DAOManager.getDAO(LegionDAO.class).setLegionKinah(legion, legion.getLegionKinah());
	...
	private void deleteLegionFromDB(Legion legion) {
		...
		for (int legionMemberObjId : legion.getLegionMembers()) {
			Player onlineLegionMember = World.getInstance().findPlayer(legionMemberObjId);
			if (onlineLegionMember != null)
                        {
                            PacketSendUtility.sendMessage(onlineLegionMember, "Votre legion vient d'etre supprimé !");
                        }
		}
	...
	private void loadLegionInfo(Legion legion) {
		 ...
		 * Load Legion Kinah
		 */
		legion.setLegionKinah(DAOManager.getDAO(LegionDAO.class).loadLegionKinah(legion));
		...
		/**
         * Load the Legion Warehouse History
         */
        DAOManager.getDAO(LegionDAO.class).loadLegionWarehouseHistory(legion);                
	}
	...
	* @return 
	*/
	public boolean useLegionWarehouse(final Player activePlayer) {
		if(canUseWarehouse(activePlayer)) {
			return true;
		}
		return false;
	}
	...
	* @return 
	*/
	public boolean canUseWarehouse(Player activePlayer) {
		if (!activePlayer.isLegionMember()) {
			PacketSendUtility.sendPacket(activePlayer, SM_SYSTEM_MESSAGE.STR_NO_GUILD_TO_DEPOSIT);
				return false;
		} else {
		//possible exploits if enable - need more checks in cm_split_item
			...
			if (!activePlayer.getLegionMember().hasRights(LegionPermissionsMask.WH_WITHDRAWAL)) {
				PacketSendUtility.sendPacket(activePlayer, new SM_SYSTEM_MESSAGE(1300322));
					return false;
			} else if (legion.isDisbanding()) {
				PacketSendUtility.sendPacket(activePlayer, SM_SYSTEM_MESSAGE.STR_GUILD_WAREHOUSE_CANT_USE_WHILE_DISPERSE);
					return false;
			}
		}
		return true;
	...
	public void addLegionKinah(Player player, long legionKinah)
	{
		Legion legion = player.getLegion();
		Storage storage = player.getInventory();
		Item item = storage.getKinahItem();
		long l = item.getItemCount();
		if (l < legionKinah)
			return;

		if (!player.getInventory().tryDecreaseKinah(legionKinah))
			return;

		//player.getInventory().decreaseKinah(legionKinah);
		String str = ItemId.KINAH.value() + ":" + legionKinah;
		getInstance().addOrRemoveWarehouseItem(player, str, true);
		legion.setLegionKinah(legion.getLegionKinah() + legionKinah);
		PacketSendUtility.sendPacket(player, new SM_LEGION_EDIT(0x04, legion));
	}

	public void getLegionKinah(Player player, long legionKinah)
	{
		if (!useLegionWarehouse(player))
			return;
		Legion legion = player.getLegion();
		if (legion.getLegionKinah() < legionKinah)
			return;
		String str = ItemId.KINAH.value() + ":" + legionKinah;
		getInstance().addOrRemoveWarehouseItem(player, str, false);
		legion.setLegionKinah(legion.getLegionKinah() - legionKinah);
		ItemService.addItem(player, 182400001, legionKinah);
		PacketSendUtility.sendPacket(player, new SM_LEGION_EDIT(0x04, legion));
	}
	...
	public void requestDisbandLegion(Creature npc, final Player activePlayer) {
		...
		if (legionRestrictions.canDisbandLegion(activePlayer, legion)) {
			...
			boolean disbandResult = activePlayer.getResponseRequester().putRequest(SM_QUESTION_WINDOW.STR_LEGION_DISBAND,
	...
	public boolean directAddPlayer(Legion legion, Player player) {
		...
		if (legion.addLegionMember(playerObjId)) {
			...
			PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_HE_JOINED(player.getName()));
	...
	private void invitePlayerToLegion(final Player activePlayer, final Player targetPlayer) {
		if (legionRestrictions.canInvitePlayer(activePlayer, targetPlayer)) {
				...
				public void acceptRequest(Creature requester, Player responder) {
					if (!targetPlayer.getCommonData().isOnline()) {
						...
						if (legion.addLegionMember(playerObjId)) {
							...
							// Tell all legion members a player joined the legion
							PacketSendUtility.sendPacket(activePlayer, SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_HE_JOINED(targetPlayer.getName()));
			...
			if (!requested) {
				...
				PacketSendUtility.sendPacket(targetPlayer, new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_LEGION_INVITE, 0,
	...
	private void appointBrigadeGeneral(final Player activePlayer, final Player targetPlayer) {
		if (legionRestrictions.canAppointBrigadeGeneral(activePlayer, targetPlayer)) {
			...
			boolean requested = targetPlayer.getResponseRequester().putRequest(SM_QUESTION_WINDOW.STR_LEGION_CHANGE_MASTER,
				responseHandler);
			// If the player is busy and could not be asked
			if (!requested) {
				...
				PacketSendUtility.sendPacket(targetPlayer, new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_LEGION_CHANGE_MASTER,
	...
	public void storeLegionEmblem(Player activePlayer, int legionId, int emblemId, int color_r, int color_g, int color_b,
		...
		if (legionRestrictions.canStoreLegionEmblem(activePlayer, legionId, emblemId)) {
			...
			PacketSendUtility.sendPacket(activePlayer, new SM_LEGION_UPDATE_EMBLEM(legionId, emblemId, color_r, color_g,
				color_b, emblemType));
			PacketSendUtility.sendPacket(activePlayer, SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_EMBLEM);
		}
	}
	...
	public void recreateLegion(Npc npc, Player activePlayer) {
		...
		if (legionRestrictions.canRecreateLegion(activePlayer, legion)) {
			...
			if (disbandResult) {
				PacketSendUtility.sendPacket(activePlayer, new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_LEGION_DISBAND_CANCEL,
					0));
			}
		}
	}
	 ...
	 * @param legionHistory
	 */
	private void addHistory(Legion legion, String text, String item, LegionHistoryType legionHistoryType) {
		LegionHistory legionHistory = new LegionHistory(legionHistoryType, text, item, new Timestamp(System.currentTimeMillis()));

		legion.addHistory(legionHistory);
		DAOManager.getDAO(LegionDAO.class).saveNewLegionHistory(legion.getLegionId(), legionHistory);

		PacketSendUtility.broadcastPacketToLegion(legion, new SM_LEGION_TABS(legion.getLegionHistory(), 0));
	}
	...
	*/
	private void addWarehouseHistory(Legion legion, String text, String item, LegionHistoryType legionHistoryType) {
		LegionHistory legionHistory = new LegionHistory(legionHistoryType, text, item, new Timestamp(System.currentTimeMillis()));

		legion.addWarehouseHistory(legionHistory);
		DAOManager.getDAO(LegionDAO.class).saveNewLegionWarehouseHistory(legion.getLegionId(), legionHistory);

		PacketSendUtility.broadcastPacketToLegion(legion, new SM_LEGION_TABS(legion.getLegionWarehouseHistory(), 2));
	}
	...
	* @param add 
	*/
	public void addOrRemoveWarehouseItem(Player player, String item, boolean add) {
		addWarehouseHistory(player.getLegion(), player.getName(), item, add == true ? LegionHistoryType.ADD_ITEM_WAREHOUSE : LegionHistoryType.TAKE_ITEM_WAREHOUSE);
	}
	...
	public void handleLegionRequest(int exOpcode, Player activePlayer) {
		switch (exOpcode) {
			...
			/** Level legion up **/
			case 0x0E:
				requestChangeLevel(activePlayer, activePlayer.getInventory().getKinah());
				break;
		}
	}
	...
	private class LegionRestrictions {
		...
		private boolean canChangeAnnouncement(LegionMember legionMember, String announcement) {
			if (!isValidAnnouncement(announcement) && legionMember.hasRights(LegionPermissionsMask.EDIT))
				return false;
			return true;
		}
