/*
 *  This file is part of Aion Extreme Emulator
 *
 ...
 */
package gameserver.services.teleport;
...
import gameserver.model.templates.spawns.SpawnSpotTemplate;
...
import gameserver.model.templates.teleport.TeleportType;
...
public class TeleportService {
	...
	private static final int TELEPORT_DEFAULT_DELAY = 2200;
        ...
        private static boolean isUseCircle;
        private static boolean isUseTeleport;
	...
	public static void teleport(TeleporterTemplate template, int locId, Player player) {
         ...
         * @param h 
         */
        public static void goInCircle(Player player, int MapId, int instanceId, float x, float y, float z, byte h, int delay)
	{
            isUseCircle = true;
        player.setTelEffect(11);
        PacketSendUtility.broadcastPacket(player, new SM_DELETE(player, 11), 50);
        PacketSendUtility.sendPacket(player, new SM_TELEPORT_LOC(MapId, x, y, z, (byte) h, 3));
		teleportTo(player, MapId, instanceId, x, y, z, h, delay, false);
	}
	/**
	...
	public static boolean teleportTo(final Player player, final int worldId, final int instanceId, final float x, final float y, final float z, final byte heading, final int delay,final boolean beam) {
		...
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				PacketSendUtility.sendPacket(player, new SM_ITEM_USAGE_ANIMATION(0, 0, 0, 0, 0, 0)); //prevent some client exploits
				changePosition(player, worldId, instanceId, x, y, z, heading);
			}
	...
	public static void teleportToPrison(Player player) {
		if (player.getRace() == Race.ELYOS)
                        goInCircle(player, WorldMapType.DE_PRISON.getId(),1, 275, 239, 49, (byte)0, 2000);
			//teleportTo(player, WorldMapType.DE_PRISON.getId(), 275, 239, 49, 3000, true);
		else if (player.getRace() == Race.ASMODIANS)
			goInCircle(player, WorldMapType.DF_PRISON.getId(),1, 275, 239, 49, (byte)0, 2000);
	}
