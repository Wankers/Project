...
public class PortalService {
	 ...
	 */
	public static void port(final PortalTemplate portalTemplate, final Player player , final int npcObjectId, int talkDelay) {
		...
		player.getController().addTask(TaskId.PORTAL, ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			@Override
			public void run() {
				PacketSendUtility.sendPacket(player, new SM_USE_OBJECT(player.getObjectId(), npcObjectId, 0, 0));
				PacketSendUtility.broadcastPacket(player, new SM_EMOTION(player, EmotionType.END_QUESTLOOT, 0, npcObjectId), true);
	...
	private static void analyzePortation(final Player player, PortalTemplate portalTemplate) {
		...
		if (portalTemplate.getKinahCount() != 0) {
			if (player.getInventory().getKinah() >= portalTemplate.getKinahCount())
				...
				PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_CANNOT_REGISTER_RESURRECT_POINT_NOT_ENOUGH_FEE);
				return;
			}
		}
			...
			switch (portalTemplate.getPlayerSize()) {
				...
				case 48:
					if (player.isInAlliance2()) {
						instance = InstanceService.getRegisteredInstance(mapId, player.getPlayerAlliance2().getTeamId());
					}
					break;
				default: // solo
