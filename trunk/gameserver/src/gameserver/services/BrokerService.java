...
import gameserver.model.broker.BrokerMessages;
...
import gameserver.network.aion.serverpackets.SM_BROKER_SERVICE;
...
public class BrokerService {
	...
	public void buyBrokerItem(Player player, int itemUniqueId) {
		...
		if (GSConfig.BROKER_PREBUY_CHECK) {
	...
	public void registerItem(Player player, int itemUniqueId, int price) {
		...
		// 3 items * 999 999 999 = integer overflow
		if (price <= 0)
			return;
		...
		PacketSendUtility.sendPacket(player, new SM_BROKER_SERVICE(newBrokerItem, 0));
	}
