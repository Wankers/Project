...
public class CubeExpandService {
	...
	public static void expand(Player player, boolean isNpcExpand) {
		if (!canExpand(player))
			return;
		PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1300431, "48")); // 9 Slots added
		if(isNpcExpand)
			player.setNpcExpands(player.getNpcExpands() + 12);
		else
			player.setQuestExpands(player.getQuestExpands() + 12);
