...
public class CraftService {
	...
	public static void finishCrafting(final Player player, RecipeTemplate recipetemplate, int critCount) {
		...
		if (player.getSkillList().addSkillXp(player, recipetemplate.getSkillid(),
			(int) RewardType.CRAFTING.calcReward(player, xpReward), recipetemplate.getSkillpoint())) {
			player.getCommonData().addExp(xpReward, RewardType.CRAFTING);
		}
		else {
			PacketSendUtility.sendPacket(player,
				SM_SYSTEM_MESSAGE.STR_MSG_DONT_GET_PRODUCTION_EXP(new DescriptionId(recipetemplate.getNameid())));
		}
	...
	private static boolean checkCraft(Player player, RecipeTemplate recipeTemplate, int skillId, VisibleObject target,
		...
		if ((skillId != 40010) && (target == null || !(target instanceof StaticObject))) {
