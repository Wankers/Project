package gameserver.services.transfers;
...
import gameserver.configs.main.PlayerTransferConfig;
...
import gameserver.model.gameobjects.player.npcFaction.ENpcFactionQuestState;
import gameserver.model.gameobjects.player.npcFaction.NpcFaction;
	...
	public Player readInfo(String name, int targetAccount, String accountName, List<Integer> rsList, Logger textLog) {
		...
		for(int a = 0; a < cnt; a++) { //inventory
			...
			if(PlayerTransferConfig.ALLOW_INV) {
				...
				Item item = new Item(newId, itemId, itemCnt, itemColor, itemCreator, itemExpireTime, itemActivationCnt, itemEquipped, itemSoulBound, unSeal, equipSlot, location, enchant, skinId, fusionId, optSocket, optFusion, charge);
