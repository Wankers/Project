...
import gameserver.network.aion.serverpackets.SM_SKILL_REMOVE;
import gameserver.skillengine.model.SkillLearnTemplate;
...
public class SkillLearnService {
	...
	public static void addNewSkills(Player player) {
            int level;
            if (CustomConfig.ENABLE_SIMPLE_2NDCLASS && !player.getPlayerClass().isStartingClass()) {
                level = 9;
            }else{
		level = player.getCommonData().getLevel();
            }
