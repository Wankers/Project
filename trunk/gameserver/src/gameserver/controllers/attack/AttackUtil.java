...
public class AttackUtil {
	...
	public static void calculatePhysicalSkillResult(Effect effect, int skillDamage, int bonus, Func func,
		...
		if (status == null) {
			status = calculatePhysicalStatus(effector, effected, true, accMod, true);
		}
	...
	public static AttackStatus calculatePhysicalStatus(Creature attacker, Creature attacked, boolean isMainHand,
		...
		if (StatFunctions.calculatePhysicalCriticalRate(attacker, attacked, isMainHand)) {
			switch (status) {
			case BLOCK:
				if (isMainHand)
		...
		if (attacked instanceof Player) {
			...
			switch (stat) {
				case PHYSICAL_CRITICAL_DAMAGE_REDUCE:
					int strikeFortitude = player.getGameStats().getStat(stat, 0).getCurrent();
					damage -= Math.round((strikeFortitude / 1000f) * damage);
					break;
				case MAGICAL_CRITICAL_DAMAGE_REDUCE:
					int spellFortitude = player.getGameStats().getStat(stat, 0).getCurrent();
					damage -= Math.round((spellFortitude / 1000f) * damage);
			}
		}
		return damage;
	}
