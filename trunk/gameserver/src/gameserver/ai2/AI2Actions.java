...
public class AI2Actions {
	...
	public static void applyEffectSelf(AbstractAI ai2, int skillId) {
		...
		Effect e = new Effect(ai2.getOwner(), ai2.getOwner(), st, 1, st.getEffectsDuration());
		e.initialize();
		e.applyEffect();
	}
	/**
	...
	public static void applyEffect(AbstractAI ai2, SkillTemplate template, Creature target) {
		Effect effect = new Effect(ai2.getOwner(), target, template, template.getLvl(), template.getEffectsDuration());
		effect.initialize();
		effect.addAllEffectToSucess();
		effect.applyEffect();
		System.out.println("Artifact use skill target: " + target.getName());
	}
