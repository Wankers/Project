...
import gameserver.world.container.PlayerContainer;
...
public class World {
	...
	public void updatePosition(VisibleObject object, float newX, float newY, float newZ, byte newHeading,
		...
		if (oldRegion == null) {
			log.warn(String.format("CHECKPOINT: oldregion is null, object coordinates - %f %f %f", object.getX(),
	...
	public void spawn(VisibleObject object) {
		...
		if (object.getSpawn() != null && object.getSpawn().isPartialDaySpawn()) {
			DayTimeSpawnEngine.addSpawnedObject(object);
		}
	}
	...
	public void despawn(VisibleObject object) {
		...
		if (oldMapRegion != null && object instanceof Creature) {
			oldMapRegion.revalidateZones((Creature) object);
                ...
                if(CustomConfig.RELATED_BEAM_DESPAWN)
                {
                    PacketSendUtility.broadcastPacket(object, new SM_DELETE(object, 2), 50);
                }
		object.clearKnownlist();
	}
       public void despawnWithCircle(VisibleObject object) {
		MapRegion oldMapRegion = object.getActiveRegion();
		if (object.getActiveRegion() != null) { // can be null if an instance gets deleted?
			if (object.getActiveRegion().getParent() != null)
				object.getActiveRegion().getParent().removeObject(object);
		...
		if (oldMapRegion != null && object instanceof Creature) {
			oldMapRegion.revalidateZones((Creature) object);
              ...
              // PacketSendUtility.broadcastPacket(object, new SM_DELETE(object, 2), 50);
                PacketSendUtility.broadcastPacket(object, new SM_DELETE(object, 11), 50);
		object.clearKnownlist();
	}
        public void despawnWithBeam(VisibleObject object) {
		MapRegion oldMapRegion = object.getActiveRegion();
		if (object.getActiveRegion() != null) { // can be null if an instance gets deleted?
			if (object.getActiveRegion().getParent() != null)
				object.getActiveRegion().getParent().removeObject(object);
