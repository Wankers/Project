 ...
 */
package gameserver.model.instance.instancereward;
...
public class PvPArenaReward extends InstanceReward<PvPArenaPlayerReward> {
	...
	public PvPArenaReward(Integer mapId, int instanceId) {
		super(mapId, instanceId);
		
		boolean isSolo = mapId == 300430000 || mapId == 300360000;
	...
	public synchronized void setRndPosition(Integer object) {
		...
		if (reward.getPosition() != 0) {
			positions.put(reward.getPosition(), Boolean.FALSE);
		}
		Integer key = getFreePositions().get(Rnd.get(getFreePositions().size()));
		positions.put(key, Boolean.TRUE);
	...
	public void portToPosition(Player player) {
		...
		Integer position = getPlayerReward(object).getPosition();
	...
	private void teleport(Player player, float x, float y, float z, byte h) {
            if(player.getWorldId() == 110070000 || player.getWorldId() == 120080000)
            {
		TeleportService.teleportTo(player, mapId, instanceId, x, y, z, h, 3000, true);
            }
            else
            {
                TeleportService.goInCircle(player, mapId, instanceId, x, y, z, h, 0);
            }
	...
	public int getNpcBonus(int npcId) {
		switch (npcId) {
			...
			case 701318: // Illusion of Hope
				return 500;
			case 701172: // Plza Flame Thrower
			case 701171: // Plza Flame Thrower
			case 701170: // Plza Flame Thrower
			case 701169: // Plza Flame Thrower
