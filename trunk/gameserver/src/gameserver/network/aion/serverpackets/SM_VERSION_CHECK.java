...
import gameserver.network.NetworkController;
...
public class SM_VERSION_CHECK extends AionServerPacket {
	...
	protected void writeImpl(AionConnection con) {
		...
		writeC(GSConfig.CHARACTER_REENTRY_TIME);
                if(GSConfig.XMAS_ENABLE || EventService.getInstance().isChristmasTime())
                {
                    writeC(0x01); //Christmas Decorations
                }
                else if(EventService.getInstance().isHalloweenTime()){ 
                    writeC(0x02); //Halloween Decorations
                }
                else{
                    writeC(0x00); //Standart
                }
