...
public class SM_CASTSPELL_RESULT extends AionServerPacket {
	...
	protected void writeImpl(AionConnection con) {
		...
		for (Effect effect : effects) {
			...
			int attackerMaxHp = effector.getLifeStats().getMaxHp();
			int attackerCurrHp = effector.getLifeStats().getCurrentHp();
			
			if (effected != null) {
				writeD(effected.getObjectId());
				writeC(0); // unk
				int targetHpPercent = 100 * effected.getLifeStats().getCurrentHp() / target.getLifeStats().getMaxHp();
				writeC(targetHpPercent); // target %hp
			}
			...
			/**
			 * Spell Status 1 : stumble 2 : knockback 4 : open aerial 8 : close aerial 16 : spin 32 : block 64 : parry 128 :
			 * dodge 256 : resist
			 */
