...
import gameserver.model.templates.item.actions.AbstractItemAction;
import gameserver.model.templates.item.actions.AssemblyItemAction;
import gameserver.model.templates.item.actions.ItemActions;
...
public class CM_USE_ITEM extends AionClientPacket {
	...
	protected void runImpl() {
		...
		ArrayList<AbstractItemAction> actions = new ArrayList<AbstractItemAction>();
                ItemTemplate part = DataManager.ITEM_DATA.getItemTemplate(item.getItemId());
                 

                /**
                 * Assembly Items
                 */
                if(part != null || part.getAid() != 0)
                {
                    if(itemActions == null){
                        itemActions = new ItemActions();
                    }
                    itemActions.getItemActions().add(new AssemblyItemAction());
                }
