...
import gameserver.ai2.handler.ShoutEventHandler;
...
import gameserver.network.aion.serverpackets.SM_CASTSPELL_RESULT;
...
import gameserver.skillengine.condition.TargetAttribute;
...
import gameserver.skillengine.properties.TargetRangeAttribute;
	...
	protected void calculateSkillDuration() {
		...
		switch (skillTemplate.getSubType()) {
			...
			case HEAL:
				...
				break;
			case ATTACK:
				if (skillTemplate.getType() == gameserver.skillengine.model.SkillType.MAGICAL) {
	...
	private boolean checkAnimationTime() {
		...
		if (motion.getInstantSkill() && hitTime == 0) {
			...
			if (clientTime < finalTime) {
				...
				// return false;
				return true; // ATTENTION!!! NEED TO BE CHECK
			}
	...
	public void useAbyssSkillAnnounce(final Player player) {
		...
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player p) {
				if (p != player && p.getWorldId() == player.getWorldId()) {
