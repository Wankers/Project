...
public abstract class EffectTemplate {
	...
	public boolean calculate(Effect effect, StatEnum statEnum, SpellStatus spellStatus) {
		...
		if (!noResist) {
			int accMod = accMod2 + accMod1 * effect.getSkillLevel();
			switch (skillType) {
				case PHYSICAL:
	...
	void afterUnmarshal(Unmarshaller u, Object parent) {
		...
		try {
			temp = EffectType.valueOf(this.getClass().getName().replaceAll("gameserver.skillengine.effect.", "").replaceAll("Effect", "").toUpperCase());
		} catch (Exception e) {
			log.info("missing effectype for "+this.getClass().getName().replaceAll("gameserver.skillengine.effect.", "").replaceAll("Effect", "").toUpperCase());
		}
