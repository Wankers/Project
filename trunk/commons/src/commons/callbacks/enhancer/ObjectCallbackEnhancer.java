...
package commons.callbacks.enhancer;
...
import commons.callbacks.util.CallbacksUtil;
import commons.callbacks.util.ObjectCallbackHelper;
import javassist.*;
...
public class ObjectCallbackEnhancer extends CallbackClassFileTransformer {
	...
	private void writeEnhancedObjectMethods(CtClass clazz) throws NotFoundException, CannotCompileException {
		...
		method.setBody("commons.callbacks.util.ObjectCallbackHelper.addCallback($1, this);");
		clazz.addMethod(method);

		method = new CtMethod(CtClass.voidType, "removeCallback", new CtClass[] { callbackClass }, clazz);
		method.setModifiers(java.lang.reflect.Modifier.PUBLIC);
		method.setBody("commons.callbacks.util.ObjectCallbackHelper.removeCallback($1, this);");
		clazz.addMethod(method);
	 ...
	 * {@link commons.callbacks.metadata.ObjectCallback} annotation, be not native and not abstract
	 * 
	 * @param method
	 *          method to check
	 * @return check result
